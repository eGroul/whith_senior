...
public:
	...
	{
		return forward.Cross((targetPosition - position).Normalized()).Normalized().y;
		// 힌트: 외적을 사용하여 방향 판별
		// 설명: targetPosition에 obj에 상대적 위치를 계산하고 이에 따른 방향 벡터를 구합니다.
		//       +X.XX, -X.XX, 0 으로 나오기 때문에 이를 정규화하여 +1,-1,0 로 리턴하게 합니다.
	}

	// TASK 4: 타겟이 시야각 내에 있는지 판별
	bool IsTargetInViewAngle(const Vector3& targetPosition) const
	{
		if (forward.Dot((targetPosition - position)) == 0)
			return true;

		short angle = acos(forward.Dot((targetPosition - position).Normalized())) * 180 /3.14;
		return angle > viewAngle/2 && angle < viewAngle + viewAngle / 2
			? true : false;
		// 힌트: 내적을 사용하여 각도 계산
		// 설명: acos을 사용하여 내적의 각도를 구합니다. (라디안 -> 도)
		//       각도가 범위 안에 들어가면 true, 안 들어가면 false를 리턴합니다.
	}
